...
function updateChatList(message, sender) {
    ...
    if (sender === username.textContent) {
        newMessageEl.classList.add('chatbox-message', 'sent');newMessageEl.innerHTML = `<a>You:</a> <p>${message}</p>`
    } else {
...
function updateMemberList(members) {
    ...
    members.forEach(member => {
        if (member.isAdmin) {
            memberListUl.innerHTML += `<div class="admin" style="color:white">${member.dataValues.name}<div class="edit-box"><button class="rmadminbtn" id="${member.dataValues.id}">Remove Admin</button>&nbsp<button class="rmuserbtn" id="${member.dataValues.id}">Remove User</button></div></div>`
        } else {
            memberListUl.innerHTML += `<div class="member">${member.dataValues.name}<div class="edit-box"><button class="makeadminbtn" id="${member.dataValues.id}">Make Admin</button>&nbsp<button class="rmuserbtn" id="${member.dataValues.id}">Remove User</button></div></div>`;
        }})
    }
